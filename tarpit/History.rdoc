...
* Drop gemspec (java) dependency on jruby-openssl. This breaks jruby
  ...
  because jruby-openssl 0.9.x is broken on jruby 1.6.x. (Recall the
  reason for the dependency in the first place was for jruby 1.6.x
  compatibility.)  Meanwhile certain RVM environments (like the ones
  used for Travis CI) pre-load jruby-openssl 0.9.x which then causes a
  conflict if tarpit declares a dependency on jruby-openssl ~> 0.8.1.

  So the only remaining path forward is to partially discontinue
  support for jruby 1.6.x. Local testing with this version should
  still work, but things like "rake push" will now require jruby 1.7
  (and a builtin, working openssl).
